...
// Deliberately a separate component from ClubOutreach, not a reskin —
// the API endpoint, generated copy, and tone are all genuinely different
// (a PE-department pitch to nominate students, never a "does your club
// have what it takes" challenge). The poster/QR/send/booking plumbing is
// identical, so this mirrors ClubOutreach's structure closely.

const PLACEHOLDER_EMAIL_HINT = /\.example\.[a-z]+$/i;

interface OutreachEmailDraft {
  subjects: string[];
  email: string;
  posterText: string;
}
...
export function SchoolOutreach({ school }: { school: Club }) {
  ...
  async function handleGenerate() {
    ...
    setSentVia(null);
    try {
      const res = await fetch("/api/ai/school-outreach-email", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ schoolId: school.id }),
      });
      if (!res.ok) throw new Error((await res.json().catch(() => null))?.error ?? "Request failed");
      const result: OutreachEmailDraft = await res.json();
      setDraft(result);
      setSubjectIdx(0);
      setBody(result.email);
    } catch (err) {
      setGenError(err instanceof Error ? err.message : "Couldn't generate the outreach email right now.");
    } finally {
      setGenerating(false);
    }
  }
  ...
  async function handleDownload() {
    ...
    try {
      const res = await fetch("/api/club-poster/pdf", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ club: school }),
      });
      if (!res.ok) throw new Error((await res.json().catch(() => null))?.error ?? "Request failed");
      ...
      a.href = url;
      a.download = `${school.name.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}-recruitment-poster.pdf`;
      a.click();
    ...
    try {
      const res = await fetch("/api/email/send", {
        ...
        body: JSON.stringify({
          ...
          message: body,
          logAs: {
            contactName: sendTo.name || school.contactName,
            organization: school.name,
            category: "CLUB",
            relatedId: school.id,
          },
          ...(attachPoster ? { attachClubPoster: { club: school } } : {}),
        }),
      });
      ...
      <CardHeader className="flex-row items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">
            Invite {school.name} to share Panna League with their students — no commitment or organizing on their
            part.
          </p>
        </div>
        ...
        <div className="space-y-3 border-t border-border pt-4">
          {!draft && (
            <Button size="sm" onClick={handleGenerate} disabled={generating}>
              ...
              {generating ? "Writing…" : "Generate outreach email"}
            </Button>
          )}
          ...
          {draft && (
            <>
                ...
                <div className="rounded-lg border border-border bg-surface-2 p-3">
                  <div className="mb-1.5 flex items-center justify-between">
                    <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
                      Poster / gym notice board caption
                    </p>
              ...
              {contacts.length > 1 ? (
                <div>
                  <label htmlFor="so-send-to" className="mb-1 block text-xs font-medium text-muted-foreground">
                    Send to
                  </label>
                  <select
                    id="so-send-to"
              ...
              {hasNoEmail && !sentVia && (
                <div className="flex items-start gap-2 rounded-lg border border-warning/30 bg-warning/5 p-2.5 text-xs text-warning">
                  ...
                  No email on file for this school yet — use Edit to add one before sending.
                </div>
              )}
              {isPlaceholderEmail && !sentVia && (
                <div className="flex items-start gap-2 rounded-lg border border-warning/30 bg-warning/5 p-2.5 text-xs text-warning">
                  <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" aria-hidden="true" />
                  This looks like a placeholder contact address — update the school&apos;s real email before sending
                  for real.
                </div>
              )}
              {!sentVia && (
